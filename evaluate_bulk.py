...
def main():
    ...
    for i in range(1, 61):
        ...
        model_D_path = './snapshots/GTA2Cityscapes/GTA5_{0:d}_D.pth'.format(i*2000)
        ...
        model_D = nn.ModuleList([FCDiscriminator(num_classes=num_class_list[i]) if i<1 else OutspaceDiscriminator(num_classes=num_class_list[i]) for i in range(2)])
        ...
        with torch.no_grad():
            for index, batch in enumerate(testloader):
                ...
                image, _, name = batch
                feat, pred = model(Variable(image).cuda(gpu0), model_D, 'target')
